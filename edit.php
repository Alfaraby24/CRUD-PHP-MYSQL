...
<h5 class="blue-text">Edit Data Anggota</h2>
<div class="row">
	<?php  
	$id = @$_GET['id'];
	$data = show("tb_anggota", "id = '$id'");
	?>
	<form class="col m6 s12" method="post">
		...
		<div class="row">
			<div class="input-field col s12">
				<input type="text" name="telp" value="<?php echo $data[0]['telp']; ?>" required />
				<label>No.Telepon</label>
			</div>
		</div>
		<div class="row">
			<div class="input-field col s12">
				<input type="submit" class="btn orange right" name="edit" value="Edit" />
			</div>
		</div>
	</form>
	<?php 
	if(@$_POST['edit']){
		...
		$alamat = @$_POST['alamat'];
		$telp = @$_POST['telp'];
		mysqli_query($db, "UPDATE tb_anggota SET nama = '$nama', jk = '$jk', alamat = '$alamat', telp = '$telp' WHERE id = '$id' ") or die ($db->error);
